...
test("finds a phrase and redacts it so it cannot be copied back out", async ({
  page,
}) => {
  const leakedRequests: string[] = [];
  const sourcePdf = await createTextPdfBytes(
    "Keep CONFIDENTIAL-TOKEN in this file",
  );
  ...
  await page.goto("/workspace#find-redact-pii");
  ...
  await page.getByLabel("Phone numbers").uncheck();
  await page.getByLabel("ID-like numbers").uncheck();
  await page.getByLabel("Custom phrase").fill("CONFIDENTIAL-TOKEN");
  await page.getByRole("button", { name: "Find matches" }).first().click();
  await expect(page.getByText("CONFIDENTIAL-TOKEN")).toBeVisible({
    timeout: 20_000,
  });
  await page.getByRole("button", { name: "Confirm all" }).click();
  await page
    .getByRole("button", { name: "Redact confirmed matches" })
    .first()
    .click();
  await expect(
  ...
  expect(download.bytes.toString("latin1")).not.toContain("CONFIDENTIAL-TOKEN");
