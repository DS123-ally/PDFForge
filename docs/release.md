...
`docker-compose.yml` runs two Node replicas (`app`, `app2`) behind nginx on port 80, each limited to 512 MB RAM and 1 CPU. `docker-compose.prod.yml` swaps in `deploy/nginx/conf.d/https.conf` and mounts `deploy/certs`.
